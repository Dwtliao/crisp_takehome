"""
Geoapify Places API Test Script
Demonstrates restaurant search with radius, category, and attribute filtering
"""
...
if not GEOAPIFY_API_KEY:
    ...
    print("Set it with: export GEOAPIFY_API_KEY='your-key-here'")
    exit(1)
# ============================================================================
...
def debug_missing_restaurants(lat: float, lon: float, restaurant_names: List[str]):
    """
    Debug function to search for specific restaurants and understand why they might not appear

    Args:
        lat: Search center latitude
        lon: Search center longitude
        restaurant_names: List of restaurant names to search for
    """
    client = GeoapifyClient(GEOAPIFY_API_KEY)

    print("\n" + "=" * 80)
    print("DEBUG: Searching for specific restaurants")
    print("=" * 80)
    ...
    for name in restaurant_names:
        ...
        # Very broad search - just restaurants, no filters
        results = client.search_places(
            lat=lat,
            lon=lon,
            radius=5000,  # Expanded to 5km to ensure we find it
        ...
        for feature in features:
            ...
            if name.lower() in place_name.lower():
                found = True
                distance = props.get('distance', 0) / 1000
                categories = props.get('categories', [])
                price_level = props.get('price_level', 'N/A')
                cuisines = props.get('cuisine', 'N/A')

                print(f"✅ FOUND: {place_name}")
                print(f"   Address: {props.get('address_line2', 'N/A')}")
                print(f"   Distance: {distance:.2f} km")
                print(f"   Categories: {', '.join(categories[:5])}")
                print(f"   Price Level: {price_level}")
                print(f"   Cuisines: {cuisines}")
                print(f"   Data completeness: {len(props)} fields")

                if distance > 2.5:
                    print(f"   ⚠️  ISSUE: Outside 2.5km walking radius (at {distance:.2f}km)")
                else:
                    print(f"   ✅ Within walking distance")

        if not found:
            print(f"❌ NOT FOUND in 5km radius")
            print(f"   Possible reasons:")
            print(f"   - Not in Geoapify database")
            print(f"   - Listed under different name")
            print(f"   - Outside search area")

    print("\n" + "=" * 80)


def hillary_cheese_prospecting(lat: float, lon: float, use_llm: bool = False, anthropic_api_key: Optional[str] = None):
    """
    Help Hillary from Happy Pastures Creamery find restaurant prospects
    for her two artisan cheese products within walking distance (2.5km).

    Product 1: Pasture Bloom Triple Crème - Fine dining, tasting menus
    Product 2: Smoky Alder Wash Rind - All upscale restaurants

    Args:
        lat: Hillary's current latitude
        lon: Hillary's current longitude
        use_llm: Whether to use LLM-based filtering (requires Anthropic API key)
        anthropic_api_key: Anthropic API key for LLM filtering
    """

    # Use provided API key or fall back to constant
    if not anthropic_api_key:
        anthropic_api_key = ANTHROPIC_API_KEY

    client = GeoapifyClient(GEOAPIFY_API_KEY, anthropic_api_key=anthropic_api_key)

    WALKING_RADIUS = 2500  # 2.5km walking distance

    print("=" * 80)
    print("HAPPY PASTURES CREAMERY - RESTAURANT PROSPECTING TOOL")
    print("=" * 80)
    print(f"\nSearching within {WALKING_RADIUS/1000}km walking radius")
    print(f"Location: {lat}, {lon}")
    print("\n" + "=" * 80)

    # Single high-quality restaurant list for BOTH cheeses
    # Hillary can pitch both products to any upscale restaurant
    print("\n🧀 HIGH-QUALITY RESTAURANT PROSPECTS (Both Cheeses)")
    print("   Products: Pasture Bloom Triple Crème + Smoky Alder Wash Rind")
    print("   Target: Fine dining, upscale casual, quality-focused restaurants")
    print("   Strategy: Broad search + quality filtering")
    print("   Includes: French, Italian, Spanish tapas, upscale seafood, steakhouses")
    print("   Excludes: Fast food, chains, obvious casual (diners, grills, taverns)")
    print("-" * 80)
    ...
    # Apply filtering - keep high-quality restaurants only
    if use_llm and anthropic_api_key:
        results = client.filter_results_with_llm(results, target_type='upscale')
    else:
        # Keyword-based filtering
        results = client.filter_results(results, target_type='fine_dining')
    ...
    prospect_count = len(results.get('features', [])) if results else 0

    print(f"\n🧀 Total High-Quality Prospects: {prospect_count}")
    print(f"   Walking Distance: {WALKING_RADIUS/1000}km (~{int(WALKING_RADIUS/1000 * 12)} minute walk)")
    print(f"\n💡 PRODUCTS TO PITCH:")
    print(f"   • Pasture Bloom Triple Crème - Fine dining applications")
    print(f"     → Tasting menus, savory pastries, cheese courses, refined plating")
    print(f"   • Smoky Alder Wash Rind - Bold flavor applications")
    print(f"     → Charcuterie, burgers, wood-fired dishes, signature flavors")
    print("\n💡 PROSPECTING TIPS:")
    print("   • Visit late afternoon (2-4pm) before dinner service")
    print("   • Avoid lunch/dinner rush hours")
    print("   • Bring samples of BOTH cheeses")
    print("   • Ask to speak with chef or kitchen manager")
    print("   • Lead with the cheese that best fits their menu style")


def example_searches(lat: float, lon: float):
    """
    Generic example searches demonstrating different features
    (Use hillary_cheese_prospecting for the business case)
    ...
    print("=" * 80)
    print("GEOAPIFY PLACES API TEST")
    print("=" * 80)

    # Example 1: Basic restaurant search within 1km
    print("\n1. BASIC RESTAURANT SEARCH (1km radius)")
    print("-" * 80)
    results = client.search_places(
        lat=LAT,
        lon=LON,
    ...
    # Example 2: Restaurants with Wi-Fi within 2km
    print("\n2. RESTAURANTS WITH WI-FI (2km radius)")
    print("-" * 80)
    results = client.search_places(
        lat=LAT,
        lon=LON,
    ...
    # Example 3: Wheelchair accessible restaurants within 1.5km
    print("\n3. WHEELCHAIR ACCESSIBLE RESTAURANTS (1.5km radius)")
    print("-" * 80)
    results = client.search_places(
        lat=LAT,
        lon=LON,
    ...
    # Example 4: Multiple filters - restaurants with Wi-Fi and outdoor seating
    print("\n4. RESTAURANTS WITH WI-FI AND OUTDOOR SEATING (2km radius)")
    print("-" * 80)
    results = client.search_places(
        lat=LAT,
        lon=LON,
        radius=2000,
        categories=['catering.restaurant'],
        filters={'wifi': True, 'outdoor_seating': True},
        limit=5
    )
    client.print_results(results)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Happy Pastures Creamery - Restaurant Prospecting Tool',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Use default Evanston, IL location (Hillary's starting point)
  python geoapify_test.py

  # Search from Chicago downtown
  python geoapify_test.py --lat 41.8781 --lon -87.6298

  # Run generic test examples instead
  python geoapify_test.py --mode test --lat 42.0451 --lon -87.6877

Business Context:
  Help Hillary find Italian/French restaurants for Pasture Bloom Triple Crème
  and gastropubs/taverns for Smoky Alder Wash Rind within walking distance.
        """
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=42.0451,
        help='Latitude of search center (default: 42.0451 - Evanston, IL)'
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=-87.6877,
        help='Longitude of search center (default: -87.6877 - Evanston, IL)'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default='hillary',
        choices=['hillary', 'test', 'debug'],
        help='Run mode: hillary (cheese prospecting), test (generic examples), or debug (find specific restaurants)'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        help='Restaurant names to debug (used with --mode debug)'
    )

    parser.add_argument(
        '--use-llm',
        action='store_true',
        help='Use LLM-based filtering instead of keyword matching (requires Anthropic API key)'
    )
    ...
    parser.add_argument(
        ...
        help='Anthropic API key for LLM filtering (or set ANTHROPIC_API_KEY env var)'
    )

    args = parser.parse_args()

    if args.mode == 'hillary':
        if args.use_llm and not args.anthropic_key:
            print("⚠️  --use-llm requires --anthropic-key or ANTHROPIC_API_KEY environment variable")
            print("Falling back to keyword-based filtering\n")

        hillary_cheese_prospecting(
            args.lat,
            args.lon,
        ...
        if not args.debug_restaurants:
            print("Error: --debug-restaurants required with --mode debug")
            print("Example: python geoapify_test.py --mode debug --debug-restaurants 'Trattoria Demi' 'Le Peep' 'LeTour'")
        else:
            debug_missing_restaurants(args.lat, args.lon, args.debug_restaurants)
    else:
        print(f"Searching near coordinates: {args.lat}, {args.lon}\n")
        example_searches(args.lat, args.lon)
